...
- **Stats THÀNH CÔNG/LỖI/THƯ MỤC luôn = 0 trong suốt "Kiểm tra trước"**: `scanFileNodes` chỉ gọi `addLog` mà không tăng `stats.*` và không gọi `updStats()`. `scanNodes` tương tự cho folder. Fix: trong `scanFileNodes` sau log OK/Lỗi → tăng `stats.copied++` hoặc `stats.failed++` + gọi `updStats()`; trong `scanNodes` worker sau `addLog('Thư mục:...')` → tăng `stats.folders++`, nếu `depth===0` tăng thêm `stats.topFolders++` + gọi `updStats()`. Copy reset stats qua `stats=ns()` ở đầu `_runCopyInternal(!isResume)` nên scan stats không lẫn vào copy.
