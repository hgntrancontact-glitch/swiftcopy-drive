...
- **Copy**: `startCopy`, `_runCopyInternal`, `copyRecTree` — đa luồng (CONCUR=12 file, FOLDER_CONCUR=5)
...
// Concurrency
const CONCUR = 12;             // file song song khi copy
const FOLDER_CONCUR = 5;       // thư mục song song khi copy
...
- **Tốc độ sao chép chậm, đặc biệt video và giai đoạn cuối**: `CONCUR=8, FOLDER_CONCUR=3` giới hạn số luồng song song. Video dùng download+reupload qua browser nên cần nhiều luồng đồng thời để tận dụng băng thông. Fix: tăng `CONCUR` từ 8 → 12, `FOLDER_CONCUR` từ 3 → 5. Backoff exponential (`sleep(2^attempt * 800ms)`) đã có sẵn cho 429/500/503 nên tăng concurrency an toàn.
...
- **Bảng báo cáo sau "Kiểm tra trước"**: Sau khi `startScan()` hoàn thành, `renderScanResult(tree, totalErr)` render panel trong `#scanResult`: tiêu đề xác nhận xong, 2 nút badge ("X sẽ copy thành công" xanh / "X sẽ lỗi" đỏ), 2 nút action ("Để tôi kiểm tra lại" đỏ nhạt → `closeScanReport()` ẩn panel; "Bắt đầu sao chép ngay →" vàng → `window.startCopy()`). Bấm badge → `toggleScanDetail(type)` toggle tree panel tương ứng (chỉ 1 panel mở cùng lúc). Tree view: `_setupScanDetailTree(type, nodes)` — mặc định chỉ mở folder root đầu tiên, các folder khác collapse, lazy render qua `buildTreeHTML+openSet`. Scan "0 lỗi" vẫn hiện panel (không chỉ render old inline "Tất cả OK").
- **Icon reset nhỏ trên header "Tiến trình đồng bộ"**: Button `onclick="window.doProgressReset()"` nằm góc phải cùng hàng header, dùng `#ic-reset` (icon vòng tròn mũi tên đã có trong SVG sprite). Chức năng: reset thanh tiến trình, stats, log, scanResult — **KHÔNG** reset inputs/checklist/session (khác `doReset()` toàn phần). Hover amber.
...
- **Video warning modal (paid)**: `#videoWarnModal` — hiện khi paid user bắt đầu copy và drive nguồn có video. Hàm `showVideoWarn(count, mode)` — `mode='before'` (nút "Đã hiểu, bắt đầu sao chép" → `confirmVideoWarn()` → `_runCopyInternal()`) và `mode='after'` (nút "Đã hiểu" → `closeVideoWarn()`). `closeComplModal()` tự động gọi `showVideoWarn(_videoFilesCount,'after')` nếu có video được copy. Phát hiện video: ưu tiên dùng `clItems` đã load (depth=0), fallback `listItems(srcId)` nếu chưa load. `#vidWarnOv` cũ là dead code, giữ lại nhưng không dùng.
