...
- **Modal "THÀNH CÔNG" thiếu toàn bộ thư mục cấp gốc sau khi copy bị ngắt đột ngột (ví dụ: Drive 403 rate limit fatal error)**: Root cause: `_runCopyInternal()` commit `topFolderNodes` vào `st.stats.folderList` bằng `for (const n of topFolderNodes) { ... }` NGAY SAU `await Promise.all(workers)` — nếu `Promise.all` throw (bất kỳ exception nào, điển hình là "Lỗi nghiêm trọng: Drive 403 - User rate limit exceeded"), JS nhảy thẳng vào catch block, dòng commit không bao giờ được thực thi → `folderList` rỗng → modal "Đã copy thành công X" chỉ hiện file lẻ ở root (đã push vào `copiedFiles` trước khi exception), toàn bộ thư mục cấp gốc biến mất. Fix: bọc `await Promise.all(workers)` trong `try/finally { for (const n of topFolderNodes) { if (n) st.stats.folderList.push(n); } }` — `finally` chạy ở MỌI đường thoát (thành công, exception, abort), đảm bảo tất cả folder đã hoàn thành (slot `topFolderNodes[i]` đã được ghi trước khi exception) luôn được commit vào `folderList` và hiện đúng trong modal.
