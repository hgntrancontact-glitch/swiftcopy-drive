...
- **Thanh tiến trình thụt lùi liên tục trong khi sao chép**: Khi `_totalDeepCount > 0` (deep scan đã biết tổng = 1575), `_progTotal` được set đúng = 1575. Nhưng `copyRecTree` và `copyRecTreeFiltered` vẫn gọi `_progTotal += items.length` mỗi lần vào subfolder → `_progTotal` tăng nhanh hơn `progDone` → `progDone/_progTotal` co lại → bar đi lùi. Fix: guard bằng `if(_totalDeepCount===0) _progTotal += items.length` trong cả hai hàm — chỉ grow dynamically khi không có pre-scan total.
