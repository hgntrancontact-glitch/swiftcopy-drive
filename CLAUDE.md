...
- **Thay đổi thiết kế: hợp nhất công thức THÀNH CÔNG/ĐÃ COPY/THƯ MỤC giữa badge sống và modal hoàn tất**: 2 nơi hiển thị cùng nhãn nhưng trước đây dùng công thức khác nhau — `updStats()` (badge sống `#sCopied`) đã có sẵn `stats.copied + stats.folders`, nhưng `showComplModal()` (modal `#complCopied`) chỉ dùng `stats.copied` (thiếu folders); `#sFolders` (badge sống) dùng `stats.topFolders` (chỉ đếm thư mục cấp gốc) trong khi `#complFolders` (modal) dùng `stats.folders` (mọi cấp độ sâu) → 2 con số lệch nhau dù cùng 1 phiên chạy, đặc biệt rõ khi có thư mục lồng nhau. Fix: cả 2 nơi giờ dùng chung công thức — **THÀNH CÔNG/ĐÃ COPY** = `stats.copied + stats.folders` (file thành công + thư mục đã tạo thành công, không cộng lỗi); **LỖI** = `stats.failed` (không đổi); **THƯ MỤC** = `stats.folders` (mọi cấp độ sâu, không tính file lẻ ở root). Bất biến `X (progDone) = THÀNH CÔNG + LỖI` đúng tại mọi thời điểm trong luồng copy thực sự vì mỗi `progInc()` đều đi kèm đúng 1 lần `stats.copied++`/`stats.failed++`/`stats.folders++` — **ngoại lệ chưa fix, nằm ngoài phạm vi yêu cầu**: nhánh bỏ qua video ở gói Free (`'Bỏ qua video (miễn phí)'`, trong `_runCopyInternal`/`copyRecTree`/`copyRecTreeFiltered`) vẫn gọi `progInc()` mà không cộng vào copied/failed/folders nào — nếu cần bất biến tuyệt đối kể cả với video bị skip, phải xử lý riêng (chưa làm vì không nằm trong 3 mục được yêu cầu sửa). `stats.topFolders` giờ không còn được đọc ở đâu (chỉ còn bị tăng tại 2 chỗ cũ) — để nguyên, không xoá, vì không thuộc phạm vi yêu cầu.
